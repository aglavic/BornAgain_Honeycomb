...
class Database(object):
    '''A database class where object members are dynamically accessed
    ...
    def __getattribute__(self, name):
        ...
        '''
        if name=='set_fake_resolve':
            return object.__getattribute__(self, 'set_fake_resolve')
        name = name.lower()
        stored_values = object.__getattribute__(self, 'stored_values')
        fake_element=object.__getattribute__(self, 'fake_element')
        fake_value=object.__getattribute__(self, 'fake_value')
        if not stored_values.has_key(name):
            try:
    ...
    def set_fake_resolve(self, name, value=1.+0j):
      '''
      Allow a fake element to be used instead of the real database,
      all others will return 0.
      '''
      object.__setattr__(self, 'fake_element', name)
      object.__setattr__(self, 'fake_value', value)

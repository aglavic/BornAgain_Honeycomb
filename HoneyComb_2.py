...
import matplotlib
import pylab
from numpy import sin, cos, arange, pi, linspace, array, ones, abs, sqrt
...
def get_sample():
    ...
    prism_ff=FormFactorPrism3(lattice_length/2., film_thickness)
    rod_ff=FormFactorBox(lattice_length/2., lattice_length/5., film_thickness)

    # first triangle is parallel to a-vector
    rotation_0=Transform3D.createRotateZ(lattice_rotation*degree)
    prism=Particle(m_particle, prism_ff, rotation_0)
    # second triangle is rotated 180 degree to first
    rotation_1=Transform3D.createRotateZ((lattice_rotation+180.)*degree)
    prism_rot=Particle(m_particle, prism_ff, rotation_1)
    # first rot is along edge connecting triangle 1 and 2
    rotation_2=Transform3D.createRotateZ((lattice_rotation+90.)*degree)
    rod_1=Particle(m_particle, rod_ff, rotation_2)
    # second rot is along edge bottom right of first triangle
    rotation_3=Transform3D.createRotateZ((lattice_rotation-30.)*degree)
    rod_2=Particle(m_particle, rod_ff, rotation_3)
    # third rot is along edge top right of second triangle
    rotation_4=Transform3D.createRotateZ((lattice_rotation+30.)*degree)
    rod_3=Particle(m_particle, rod_ff, rotation_4)

    pos0=kvector_t(0.0, 0.0, 0.0)
    # move first triangle up one edge length, include lattice rotation
    pos1=kvector_t(-sin(lattice_rotation*degree)*lattice_length,
                   cos(lattice_rotation*degree)*lattice_length, 0.)
    # move first rod up one half edge length, include lattice rotation
    pos2=kvector_t(-sin(lattice_rotation*degree)*lattice_length/2.,
                   cos(lattice_rotation*degree)*lattice_length/2., 0.)
    # move second rod bottom left one half edge length (-30 edge direction)
    pos3=kvector_t((cos(lattice_rotation*degree)*sin(-30.*degree)
                   -sin(lattice_rotation*degree)*cos(-30.*degree)
                   )*lattice_length/2,
                   (sin(lattice_rotation*degree)*sin(-30.*degree)+
                    cos(lattice_rotation*degree)*cos(-30.*degree)
                    )*lattice_length, 0.)
    # move third rod up one edge length and right one half edge length
    pos4=kvector_t((cos(lattice_rotation*degree)*sin(30.*degree)
                   -sin(lattice_rotation*degree)*(cos(30.*degree)+2.)
                   )*lattice_length/2,
                   (sin(lattice_rotation*degree)*sin(30.*degree)+
                    cos(lattice_rotation*degree)*(cos(30.*degree)+2.)
                    )*lattice_length, 0.)
    basis=LatticeBasis()
    basis.addParticle(prism, [pos0])
    basis.addParticle(prism_rot, [pos1])
    basis.addParticle(rod_1, [pos2])
    basis.addParticle(rod_2, [pos3])
    basis.addParticle(rod_3, [pos4])
    particle_layout=ParticleLayout()
    particle_layout.addParticle(basis)

    interference=InterferenceFunction2DLattice.createHexagonal(lattice_length*sqrt(3.),
